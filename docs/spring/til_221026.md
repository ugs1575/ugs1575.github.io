...
- @Scope(value = "request") -> request스코프로 지정했다.
...
- 중요한 점!
  ...
  - 웹과 관련된 부분은 컨트롤러까지만 사용해야한다. 서비스 계층은 웹 기술에 종속되지 안혹, 가급적 순수하게 유지하는 것이 유지보수 관점에서 좋다.
...
- 그리고 myLoggerProvider.getObject(); 시점에는 HTTP요청이 진행이므로 request scope빈의 생성이 처리된다.
- getObject()를 controller와 service에서 각각 호출해도 같은 HTTP 요청이면 같은 스프링 빈이 반환된다.
...
### 동작 방식
- myLogger.getClass()로 확인해보면 CGLIB를 볼수 있다.
...
- 가짜 프록시 객체는 요청이 오면 그때 내부에서 진짜 빈을 요청하는 위임 로직이 들어있다.
  - 클라이언트가 myLogger.logic()을 호출하면 사실은 가짜 프록시 객체의 메서드를 호출한 것이고 가짜 프록시 객체가 진짜 logic메서드를 호출한다.
