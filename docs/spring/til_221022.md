...
```
- Map<String, DiscountPolicy> policyMap : map의 키에 스프링 빈의 이름을 넣어주고, 그 값으로 DiscountPolicy 타입으로 조회한 모든 스프링 빈을 담아준다.
- List<DiscountPolicy> policies : DiscountPolicy타입으로 조회한 모든 스프링 빈을 담아준다. 만약 해당하는 타입의 스프링 빈이 없으면, 빈 컬렉션이나 Map을 주입한다.
...
- 업무 로직은 숫자도 많고 유사항 패턴이 있다 -> 자동기능 활용
